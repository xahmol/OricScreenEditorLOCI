...
- **`divu16by8` runtime (oscar64 ≥ v1.32.272+41):** commit `5da792a` added a fast
  `uint16÷uint8` path; `crt_math.c` needs a `DM8:` label in `__asm divmod` and
  `#pragma runtime(divu16by8, divmod.DM8)`. Missing either → `error 3002: Missing
  runtime code implementation 'divu16by8'`. See `oscar64manual.md` for the fix.
...
**Phosphoric CAN emulate a LOCI device** (v1.50+). **Oricutron cannot.** Use
