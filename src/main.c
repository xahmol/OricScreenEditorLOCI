...
void plotvisible(unsigned char row, unsigned char col, unsigned char setorrestore)
{
    ...
    if (row >= yoffset && row <= yoffset + 26 && col >= xoffset && col <= xoffset + 39)
    {
        ...
        {
            enable_overlay_ram();
            val = PEEK(screenmap_screenaddr(row, col, screenwidth));
            disable_overlay_ram();
            cputcxy(col - xoffset, row - yoffset, val);
        }
    }
}
...
unsigned char filenamegrabber(unsigned char xpos, unsigned char ypos)
{
    ...
    {
        valid = 1;
        if (filename[x - 2] == 'P' && filename[x - 1] == 'J')
        {
            valid = 2;
        }
        if (filename[x - 2] == 'S' && filename[x - 1] == 'C')
        {
            valid = 3;
        }
        if (filename[x - 2] == 'C' && filename[x - 1] == 'S')
        {
            valid = 4;
        }
        if (filename[x - 2] == 'C' && filename[x - 1] == 'A')
        {
            valid = 5;
        }
    }
